...
const Testimonial = ({ testimonialText, types, companyName, personName, expanded, onToggle, companylink, profilelink, img }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`transition-all duration-500 ease-in-out ${expanded ? 'opacity-100 max-h-none mt-6' : 'opacity-0 max-h-0 mt-0'} overflow-hidden px-[3vw]`}
        style={{ pointerEvents: expanded ? 'auto' : 'none' }}
      >
        <div className="flex flex-col lg:flex-row gap-5 lg:gap-[4vw] items-start w-full min-w-0">
          <div className='flex flex-row lg:flex-col gap-8 lg:gap-[4vw] items-center lg:items-start justify-center min-w-0'>
          ...
          <div className="flex flex-col w-full lg:w-1/3 lg:gap-[4vw] gap-8 min-w-0">
            <img
              src={img}
              alt="img"
              className="w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-[16vw] h-auto object-cover rounded-lg"
            />
