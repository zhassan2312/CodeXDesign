    ...
    >
      ...
      <div className='aboutMiddleSection flex flex-col justify-start items-start md:flex-row md:justify-between px-[3vw] gap-[3vw]'>
        ...
        <p className='flex flex-col gap-[1vw]'>
          ...
          {socialMediaLinks.map((link, index) => (
            <Link className={''} isBlack={true} text={link.name} href={link.to.startsWith('http') ? link.to : `https://${link.to}`} key={index} isNavbar={false} />
          ))}
        </p>
      </div>
      ...
      <div className='w-full aboutEndSection mx-[3vw] flex flex-col md:flex-row items-start justify-between gap-8 overflow-hidden'>
        <div className='flex flex-col items-start w-auto'>
          ...
          <Button href={'https://www.linkedin.com/in/zhassan2312/'} className={'pl-8 pr-16 lg:pl-[2vw] lg:pr-[4vw] py-[1vw]'} text={'CHECK OUT!'} isWhite={false} />
        </div>
        <img src={assets.myImage} className='w-[93vw] md:w-1/2 h-[70vh] rounded-3xl md:mr-[6vw] fill object-cover overflow-hidden' alt="About" />
      </div>
    </div>
  );
};
